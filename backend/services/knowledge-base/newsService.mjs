...
export const getNewsFeed = async ({ category = "all", limit = 20 } = {}) => {
  ...
  if (!db) {
    ...
    return { items: filtered, updatedAt: filtered[0]?.updatedAt || null, source: "memory" };
  }
  const filter = category === "all" ? {} : { category };
  const items = await db.collection(COLLECTION_NEWS).find(filter).sort({ publishedTs: -1 }).limit(limit).toArray();
  const state = await db.collection(COLLECTION_STATE).findOne({ key: "news_feed" });
  return { items, updatedAt: state?.updatedAt || null, source: "db" };
};
