      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section data-reveal className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <aside className="rounded-[28px] border border-white/8 bg-[#090d14] p-5 xl:sticky xl:top-24 xl:self-start">
            ...
            <div className="mt-5 space-y-3">{leaderboard.length ? leaderboard.map((row) => <div key={`${row.alias}-${row.position}`} className="rounded-2xl border border-white/8 bg-black/20 p-3"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-white">{row.position}. {row.alias}</p><p className="mt-1 text-xs text-slate-400">{row.rank} | Level {row.level}</p></div><span className="rounded-full border border-blue-400/16 px-2.5 py-1 text-[11px] text-blue-100">{row.points} pts</span></div></div>) : <div className="rounded-2xl border border-white/8 bg-black/20 p-6 text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border border-amber-400/18 bg-amber-400/8"><span className="text-2xl">🏆</span></div><p className="font-semibold text-white">No rankings yet</p><p className="mt-2 text-sm text-slate-400">Complete labs to earn XP and rank up!</p><button type="button" className="mt-4 inline-flex items-center gap-2 rounded-full border border-emerald-400/18 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-100 transition hover:bg-emerald-400/16" onClick={() => { const firstLab = labs[0]; if (firstLab) { loadSelectedLab(firstLab); setLabModalOpen(true); } }}>Start a Lab</button></div>}</div>
          </aside>
        </section>
        {labLoadError ? <div className="rounded-2xl border border-amber-300/15 bg-[#090d14] px-4 py-4 text-sm text-slate-200"><span className="block">{labLoadError}</span><button type="button" className="mt-2 text-xs text-blue-100 underline transition hover:text-blue-50" onClick={() => window.location.reload()}>Retry</button></div> : null}

        {!labs.length ? (
          <div className="rounded-2xl border border-white/8 bg-[#090d14] px-6 py-10 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-blue-400/18 bg-blue-500/8"><span className="text-3xl">🔬</span></div><h2 className="text-xl font-semibold text-white">Labs Launching Soon</h2><p className="mt-3 text-sm text-slate-300 max-w-md mx-auto">Stay tuned for interactive cyber labs covering web security, network defense, forensics, OSINT, and CTF challenges.</p><button type="button" className="mt-5 inline-flex items-center gap-2 rounded-full border border-emerald-400/18 bg-emerald-400/10 px-5 py-2.5 text-sm font-medium text-emerald-100 transition hover:bg-emerald-400/16" onClick={() => window.location.href = "/mission-control"}>Try Missions Instead</button></div>
        ) : activeLab ? (
