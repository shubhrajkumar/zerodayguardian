...
const COSTS = {
  openai: env.openaiCostPer1k,
  google: env.googleCostPer1k,
};
...
const normalizeRequest = (request = {}) => {
  ...
  return {
    ...
    timeoutMs: Math.max(1000, Math.min(60_000, Number(request.timeoutMs || env.llmCriticalTimeoutMs))),
  ...
  const error =
    code === "invalid_request"
      ...
      : code === "auth" || code === "env" || code === "invalid_model"
        ...
        : new Error("AI services are temporarily busy. Please try again shortly.");
  error.code = code;
  error.provider = "";
  ...
  return queueGuard(request, "generate", () => withSpan("llm.request", { user_tier: request.userTier }, async () => {
    ...
    for (const provider of orderedProviders) {
      ...
      for (let attempt = 0; attempt <= env.llmMaxRetries; attempt += 1) {
        ...
        const timeout = withTimeoutSignal(request.timeoutMs, rawRequest.signal);
        try {
          ...
          const canRetry = failure.retryable && attempt < env.llmMaxRetries;
          ...
          if (isAbort(tracked.error)) throw tracked.error;
          if (!canRetry) break;
    ...
    for (const provider of orderedProviders) {
      ...
      for (let attempt = 0; attempt <= env.llmMaxRetries; attempt += 1) {
        ...
        try {
          ...
          lastError = tracked.error;
          exhaustedFailures.push(compactFailure(provider, tracked.error, failure, attempt + 1));
          const canRetry = !emittedAnyToken && failure.retryable && attempt < env.llmMaxRetries;
          const retryDelayMs = canRetry ? backoffMs(attempt, failure, tracked.error) : 0;
