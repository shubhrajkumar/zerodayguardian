...
export type ToolDefinition = (typeof toolsData)[number];

export const toolsCatalog = toolsData as ToolDefinition[];
...
export const getToolById = (id: number) => toolsCatalog.find((tool) => tool.id === id);

export const relatedToolsFor = (tool: ToolDefinition, limit = 3) =>
  toolsCatalog.filter((item) => item.id !== tool.id && item.category === tool.category).slice(0, limit);
