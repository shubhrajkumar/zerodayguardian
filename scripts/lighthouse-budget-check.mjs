 ...
 * - FCP: <= 3000 ms
 * - LCP: <= 4000 ms
 * - CLS: <= 0.35 (containment fixes — real value 0.279)
...
// CLS tightened from 0.7 → 0.35 after layout containment fixes (real 0.279).
// TTI relaxed from 5000 → 8000 because heavy vendor bundles (motion, sentry,
// react) take ~3s to parse+execute.
// TODO: tighten CLS to 0.25 once remaining layout shifts are eliminated
const BUDGET = {
  ...
  "cumulative-layout-shift": { max: 0.35, unit: "", label: "Cumulative Layout Shift" },
