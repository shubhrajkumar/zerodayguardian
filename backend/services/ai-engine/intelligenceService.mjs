...
const PROMPT_POOL = [
  ...
  },
  {
    ...
    nextStep: "Classify ten synthetic alerts and measure triage consistency.",
  },
  {
    id: "prompt_career_1",
    intent: "learning",
...
export const getDashboardIntelligence = async (actor) => {
  ...
  const achievements = [
    Number(profile.xp || 0) >= 200 ? "First 200 XP" : null,
    Number(profile.streak || 1) >= 5 ? "5-day Streak" : null,
    completedLabs >= 3 ? "Lab Runner" : null,
    telemetry.toolsUsed >= 4 ? "Toolkit Explorer" : null,
  ].filter(Boolean);
...
export const analyzeHeadersFromUrl = async (actor, targetUrl) => {
  ...
  try {
    ...
    const aborted = String(fetchError?.name || "").toLowerCase() === "aborterror";
...
export const simulateSubdomainRecon = async (actor, target) => {
  ...
  if (!isValidPublicDomain(host)) {
    const error = new Error("Invalid target domain");
    error.status = 400;
    throw error;
  }
  ...
  const max = 10 + (parseInt(seed.slice(0, 2), 16) % 6);
  const selected = [];
  for (let i = 0; i < wordlist.length && selected.length < max; i += 1) {
    const nibble = parseInt(seed.slice(i * 2, i * 2 + 2) || "0", 16);
    if (nibble % 3 !== 0) selected.push(wordlist[i]);
  }
  if (!selected.includes("api")) selected.unshift("api");
  const subdomains = [...new Set(selected)].map((prefix, idx) => {
    const confidence = 55 + (parseInt(seed.slice(idx + 4, idx + 6), 16) % 45);
    const exposure = /(admin|dev|staging|backup|db|qa|vpn)/.test(prefix) ? "elevated" : "moderate";
    return {
  ...
  await recordTelemetryEvent(actor, {
    ...
    query: host,
    tool: "subdomain-recon-simulator",
    depth: 2,
    success: true,
  ...
  return {
    ...
    subdomains,
    methodology:
      "Deterministic simulation uses domain-derived seed + common infrastructure naming patterns to model likely external subdomain exposure.",
    defensiveAdvice: [
      "Continuously inventory DNS records and decommission stale hosts.",
      "Apply least-privilege and authentication controls on admin/staging subdomains.",
      "Monitor certificate transparency logs for unexpected hostname issuance.",
    ],
  };
};
