  ...
  return (
    <div className="container mx-auto px-4 py-12 page-shell">
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-[32px] border border-blue-400/12 bg-[var(--theme-bg)] p-1">
          <PlatformHero
            eyebrow="Cyber Range"
            title={<span className="text-[var(--theme-text)]">Practice Real Cyber Skills</span>}
        ...
        {activeLab && hasActiveLabSession ? (
          <section data-reveal className="flex flex-col gap-3 rounded-[24px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Active Lab</p><h2 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{activeLab.title}</h2><p className="mt-1 text-sm text-slate-300">Resume with {missionProgress}% progress and {currentMissionScore} score.</p></div>
        ...
        <section data-reveal className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filteredLabs.slice(0, 6).map((lab) => {
                ...
                return (
                  <article key={lab.id} className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5 transition duration-200 hover:-translate-y-1 hover:border-blue-400/18">
                    <div className="flex items-start justify-between gap-3"><div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-blue-400/14 bg-blue-500/10 text-blue-200"><CategoryIcon className="h-5 w-5" /></div><span className="rounded-full border border-[var(--theme-border)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-slate-300">{String(lab.level || "intermediate").toUpperCase()}</span></div>
                    <p className="mt-4 text-[11px] uppercase tracking-[0.18em] text-slate-500">{category}</p>
                    <h3 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{lab.title}</h3>
                    <p className="mt-3 text-sm leading-6 text-slate-300">{lab.description}</p>
                    <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] px-3 py-3"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">XP Reward</p><p className="mt-2 font-semibold text-[var(--theme-text)]">+{xpReward}</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] px-3 py-3"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Time</p><p className="mt-2 font-semibold text-[var(--theme-text)]">{timeEstimate} min</p></div></div>
          ...
          <aside className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5 xl:sticky xl:top-24 xl:self-start">
            <div className="flex items-center justify-between gap-2"><div><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Leaderboard</p><h2 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">Top 5 This Week</h2></div><span className="rounded-full border border-[var(--theme-border)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-slate-300">Weekly</span></div>
            <div className="mt-5 space-y-3">{leaderboard.length ? leaderboard.map((row) => <div key={`${row.alias}-${row.position}`} className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-3"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-[var(--theme-text)]">{row.position}. {row.alias}</p><p className="mt-1 text-xs text-slate-400">{row.rank} | Level {row.level}</p></div><span className="rounded-full border border-blue-400/16 px-2.5 py-1 text-[11px] text-blue-100">{row.points} pts</span></div></div>) : <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-6 text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border border-amber-400/18 bg-amber-400/8"><span className="text-2xl">🏆</span></div><p className="font-semibold text-[var(--theme-text)]">No rankings yet</p><p className="mt-2 text-sm text-slate-400">Complete labs to earn XP and rank up!</p><button type="button" className="mt-4 inline-flex items-center gap-2 rounded-full border border-emerald-400/18 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-100 transition hover:bg-emerald-400/16" onClick={() => { const firstLab = labs[0]; if (firstLab) { loadSelectedLab(firstLab); setLabModalOpen(true); } }}>Start a Lab</button></div>}</div>
          </aside>
        </section>
        ...
        {!labs.length ? (
          ...
          <section data-reveal className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="space-y-4">
              <article className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Mission Board</p><h2 className="mt-2 text-2xl font-semibold text-[var(--theme-text)]">{activeLab.title}</h2><p className="mt-2 text-sm leading-6 text-slate-300">{activeLab.objective}</p></div><div className="flex flex-wrap gap-2"><span className="rounded-full border border-[var(--theme-border)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-slate-300">{progression?.rank || "Active"}</span><span className={`rounded-full border px-3 py-1 text-[11px] ${dynamicDifficulty.accent}`}>{dynamicDifficulty.label}</span></div></div>
                <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Progress</p><p className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{missionProgress}%</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--theme-overlay)]"><div className="h-full rounded-full bg-[#2f81ff] transition-all duration-500" style={{ width: `${missionProgress}%` }} /></div></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Score</p><p className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{currentMissionScore}</p><p className="mt-2 text-xs text-slate-400">x{scoringMultiplier.toFixed(1)} mode multiplier</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Estimate</p><p className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{timerLabel}</p><p className="mt-2 text-xs text-slate-400">{activeLab.timerMinutes || activeLab.estimatedMinutes || 12} min target</p></div></div>
                <div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Current Objective</p><p className="mt-2 text-sm font-medium text-[var(--theme-text)]">{(missionState || fallbackMissionState)?.current_objective || activeLab.steps[0]}</p><p className="mt-2 text-xs text-slate-400">Next action: {(missionState || fallbackMissionState)?.next_action || "help"}</p><p className="mt-2 text-xs text-slate-400">Expected outcome: {(missionState || fallbackMissionState)?.expected_outcome || "Validate the next mission step to progress."}</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Simulation Mode</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => startTransition(() => setMindset("defense"))} className={`rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] transition ${isDefense ? "border-blue-400/40 bg-blue-500/10 text-blue-50" : "border-[var(--theme-border)] bg-[var(--theme-overlay)] text-slate-300"}`}>Defense</button><button type="button" onClick={() => startTransition(() => setMindset("offense"))} className={`rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] transition ${!isDefense ? "border-blue-400/40 bg-blue-500/10 text-blue-50" : "border-[var(--theme-border)] bg-[var(--theme-overlay)] text-slate-300"}`}>Offense</button></div><p className="mt-3 text-sm leading-6 text-slate-300">{dynamicDifficulty.detail || ''}</p></div></div>
              ...
              <article className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Attack / Defense Simulation</p><h3 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">Scenario breakdown</h3></div><span className={`rounded-full border px-3 py-1 text-[11px] ${missionRiskTone}`}>{missionFeedback?.riskLevel || "LOW"}</span></div><div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Narrative</p><p className="mt-2 text-sm leading-6 text-slate-300">{activeLab.attackNarrative || activeLab.description}</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Operator feedback</p><p className="mt-2 text-sm leading-6 text-slate-300">{missionFeedback?.operatorAction || "ZORVIX feedback appears here after command execution."}</p>{(() => {
            ...
            <div className="space-y-4">
              <article className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Recommendations</p><h3 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">Real next actions</h3></div><button type="button" onClick={() => loadRecommendations().catch(() => undefined)} className="rounded-full border border-blue-400/16 bg-blue-500/10 px-3 py-1 text-[11px] font-medium text-blue-100 transition hover:bg-blue-500/16" disabled={pyRecLoading}>{pyRecLoading ? "Refreshing..." : "Refresh"}</button></div>{pyRecError ? <p className="mt-3 text-xs text-rose-300">{pyRecError}</p> : null}<div className="mt-4 space-y-3">{pyRecommendations?.recommendations?.length ? pyRecommendations.recommendations.slice(0, 3).map((item) => <div key={`${item.title}-${item.action}`} className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="font-medium text-[var(--theme-text)]">{item.title}</p><p className="mt-2 text-sm leading-6 text-slate-300">{item.reason}</p><p className="mt-2 text-sm text-blue-100">Action: {item.action}</p></div>) : recommendedLabs.length ? recommendedLabs.map((lab) => <button key={lab.id} type="button" onClick={() => loadSelectedLab(lab)} className="w-full rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4 text-left transition hover:border-blue-400/18"><div className="flex items-center justify-between gap-3"><p className="font-medium text-[var(--theme-text)]">{lab.title}</p><span className="text-[11px] uppercase tracking-[0.16em] text-slate-400">{lab.level || "mission"}</span></div><p className="mt-2 text-sm leading-6 text-slate-300">{lab.objective}</p></button>) : <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4 text-sm text-slate-300">Verified recommendations appear as you complete more labs.</div>}</div></article>

              <article className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Lab Monitor</p><h3 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">State, rewards, and rank</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Momentum</p><p className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{momentumPercent}%</p><p className="mt-2 text-xs text-slate-400">Streak {progression?.streak || 0} · {accentLabel}</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Overall progress</p><p className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{progressPct}%</p><p className="mt-2 text-xs text-slate-400">{completedLabs} completed / {Math.max(totalLabsTouched, labs.length)} touched</p></div></div><div className="mt-4 flex flex-wrap gap-2">{badges.map((badge) => <span key={badge} className="rounded-full border border-blue-400/14 px-3 py-1 text-[11px] text-blue-100">{badge}</span>)}</div>{missionPulse ? <div className={`mt-4 rounded-2xl border px-4 py-3 text-sm ${missionPulse.tone === "success" ? "border-emerald-400/18 bg-emerald-400/8 text-emerald-100" : missionPulse.tone === "warning" ? "border-amber-400/18 bg-amber-400/8 text-amber-100" : "border-blue-400/18 bg-blue-500/8 text-blue-100"}`}><p className="font-medium">{missionPulse.title}</p><p className="mt-1 text-xs opacity-85">{missionPulse.detail}</p></div> : null}<div className="mt-4 space-y-3">{recentRewards.slice(0, 2).map((reward) => <div key={reward.id} className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="font-medium text-[var(--theme-text)]">{reward.title}</p><p className="mt-2 text-xs text-slate-400">{reward.detail || ''}</p></div>)}{!recentRewards.length ? <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4 text-sm text-slate-300">Rewards and debrief snapshots appear after successful runs.</div> : null}</div></article>

              <article className="rounded-[28px] border border-[var(--theme-border)] bg-[var(--theme-surface)] p-5"><p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Debrief</p><h3 className="mt-2 text-xl font-semibold text-[var(--theme-text)]">{latestDebrief.mission}</h3><p className="mt-3 text-sm leading-6 text-slate-300">{latestDebrief.headline}</p><div className="mt-4 grid gap-3"><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Branch</p><p className="mt-2 text-sm text-[var(--theme-text)]">{branchSuggestion?.title || "Primary branch not locked yet"}</p></div><div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-overlay)] p-4"><p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Next step</p><p className="mt-2 text-sm text-[var(--theme-text)]">{latestDebrief.nextStep}</p></div></div></article>
            </div>
          </section>
        ) : null}
      </div>
