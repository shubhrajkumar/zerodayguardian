...
const pickLabel = (mode = "", seed = 0) => {
  ...
  if (normalized === "cybersecurity") {
    return bucket === 0 ? { title: "Security Answer", next: "Validation", key: "Key Signals", risk: "Risk" } : bucket === 1
  ...
  if (normalized === "file_analysis") {
    return bucket === 0 ? { title: "Quick Analysis", next: "Validation", key: "Key Signals", risk: "Risk" } : bucket === 1
...
export const formatAssistantResponse = ({ text = "", mode = "general", variantSeed = "" } = {}) => {
  ...
  const explicitBullets = extractBullets(lines.filter((line) => /^[-*•]/.test(line)));
  ...
  const sections = [];
  if (raw.length <= 120 && !explicitBullets.length) {
    return raw;
  }
