...
import { classifyAiError, routeAiRequest, routeAiStreamRequest, verifyAiEngine } from "../../src/ai-engine/index.mjs";
import {
...
import { logError, logWarn } from "../../src/utils/logger.mjs";
...
const buildLlmFailureState = (error = null) => {
  ...
  if (
    hasLocalFallbackText &&
    !["auth", "env", "invalid_model", "bad_request", "invalid_request", "forced_local_fallback"].includes(code)
  ) {
    code = "service_busy";
  }
  ...
  if (code === "service_busy") {
    return {
      code,
      title: hasLocalFallbackText ? "Local fallback response" : "AI services are temporarily busy",
      ...
      provider: hasLocalFallbackText ? "local" : "",
      retryable: true,
      retryAfterSec,
      retryHint: retryAfterSec ? `Retry in about ${formatRetryAfter(retryAfterSec)}.` : "Please try again shortly.",
  ...
  if (code === "timeout") {
    return {
      ...
      detail: `The AI provider did not finish generating within the timeout window.${providerHint}`.trim(),
      provider,
      retryable,
      retryAfterSec,
      retryHint,
...
const buildEmergencyAssistantReply = (message = "", topic = null, llmError = null) => {
  ...
  const topicHint = topicTitle ? ` for ${topicTitle}` : "";
  if (llmError) {
    if (llmError.code === "service_busy") {
      return "AI services are temporarily busy. Please try again shortly.";
    }
    return [
      `${llmError.title}${topicHint}.`,
      clean ? `Your request was received${topicHint}.` : "Your request was received.",
      llmError.detail,
      llmError.retryHint,
    ].join(" ");
  }
  if (!clean) {
    return `Zorvix could not reach the AI service${topicHint}. Please retry in a moment.`;
  }
  return [
    `Your request was received${topicHint}.`,
    "The AI provider is temporarily unavailable, so I could not generate a full answer.",
    "Retry in a moment. If this keeps happening, check provider connectivity, API key settings, and quota status.",
  ].join(" ");
};
...
router.post("/chat", validateBody(chatSchema), async (req, res, next) => {
  try {
    ...
    if (forcedLocalFallback()) {
      ...
      assistantText = await routeAiRequest({
...
router.post("/chat/stream", validateBody(chatStreamSchema), async (req, res) => {
  ...
  try {
    ...
    if (forcedLocalFallback()) {
      ...
      const fallbackText = await routeAiRequest({
        messages: [...conversation.messages.slice(-18), userMessage],
        topic: conversation.activeTopic,
        assistantProfile,
        correlationId: req.requestId,
