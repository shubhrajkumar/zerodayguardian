...
const aiRequestPath = (req) => /\/api\/(ai|neurobot\/chat|neurobot\/llm)/.test(String(req?.originalUrl || req?.path || ""));
...
const buildAiErrorState = (err) => {
  ...
  if (code === "timeout") {
    return {
      ...
      detail: `The AI provider did not finish generating before the timeout window.${providerHint}`.trim(),
      retryable: true,
      retryAfterSec,
  ...
  if (code === "service_busy") {
    return {
      code,
      title: "AI services are temporarily busy",
      detail: "AI services are temporarily busy. Please try again shortly.",
      retryable: true,
      retryAfterSec,
      retryHint: retryAfterSec ? `Retry in about ${Math.max(1, Math.ceil(retryAfterSec))}s.` : "Please try again shortly.",
      provider: "",
      statusCode: aiHttpStatus(code, failure.statusCode || err?.statusCode || err?.status || 503),
    };
  }
...
export const errorHandler = (err, req, res, _next) => {
  ...
  res.status(safeStatus).json({
    ...
    ...(env.nodeEnv !== "production" ? { debug: String(err?.message || err) } : {}),
  });
};
