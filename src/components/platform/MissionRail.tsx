  ...
  return (
    <section data-reveal className="grid gap-4 xl:grid-cols-4">
      {rails.map((track) => (
        ...
        >
          <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-100/85">{track.level} rail</p>
          ...
          <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.03] px-3 py-3 text-xs">
            <p className="text-cyan-100/85">Deploy mode</p>
            <p className="mt-1 text-slate-100">{track.mode}</p>
            <p className="mt-3 text-cyan-100/85">Intel payoff</p>
            <p className="mt-1 text-slate-300/85">{track.payoff}</p>
            <p className="mt-3 text-cyan-100/85">Breach status</p>
