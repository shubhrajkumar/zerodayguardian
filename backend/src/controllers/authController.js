...
import { assertAuthAttemptAllowed, recordAuthFailure, recordAuthSuccess } from "../services/authThreatService.mjs";
...
const toPublicUser = (user) =>
  user
      ? {
        ...
        authProvider: String(user.authProvider || "google"),
...
const toTokenBackedUser = (authUser) =>
  authUser
    ? {
        ...
        authProvider: String(authUser?.authProvider || "google"),
...
export const login = async (req, res) => {
  try {
    await assertAuthAttemptAllowed({ req, identifier: req.validatedBody?.email || "login" });
    ...
    await safeRecordAuthFailure({ req, identifier: req.validatedBody?.email || "login", reason: error?.code || "login_failed" });
...
export const verifyOtp = async (req, res) => {
  try {
    logInfo("Auth verify OTP request", { requestId: req.requestId || "", email: req.validatedBody?.email || "" });
    const { verified, user } = await verifyUserOtp(req.validatedBody);
    res.json({ status: "ok", verified, user: toPublicUser(user) });
  } catch (error) {
    logError("Auth verify OTP failed", error, { requestId: req.requestId || "", email: req.validatedBody?.email || "" });
    sendAuthError(req, res, error);
  }
};
...
export const forgotPassword = async (req, res) => {
  try {
    await assertAuthAttemptAllowed({ req, identifier: req.validatedBody?.email || "forgot-password" });
    logInfo("Auth forgot password request", { requestId: req.requestId || "", email: req.validatedBody?.email || "" });
    const result = await requestPasswordReset(req.validatedBody);
    // Always return 200 to prevent user enumeration
    res.json({ status: "ok", message: result.message });
  } catch (error) {
    await safeRecordAuthFailure({ req, identifier: req.validatedBody?.email || "forgot-password", reason: error?.code || "forgot_password_failed" });
    logError("Auth forgot password failed", error, { requestId: req.requestId || "", email: req.validatedBody?.email || "" });
    sendAuthError(req, res, error);
  }
};
...
export const resetPasswordHandler = async (req, res) => {
  try {
    await assertAuthAttemptAllowed({ req, identifier: req.validatedBody?.email || "reset-password" });
    logInfo("Auth reset password request", { requestId: req.requestId || "", email: req.validatedBody?.email || "" });
    const result = await resetPassword(req.validatedBody);
    res.json({ status: "ok", message: result.message });
  } catch (error) {
    await safeRecordAuthFailure({ req, identifier: req.validatedBody?.email || "reset-password", reason: error?.code || "reset_password_failed" });
